...
async function importCSV() {
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i += batchSize) {
      ...
      console.log(`Processando lote ${Math.floor(i / batchSize) + 1}/${Math.ceil(lines.length / batchSize)}...`);
      ...
      try {
        for (const line of batch) {
          ...
          if (!data.cnpj_basico) {
            console.warn('Linha ignorada - CNPJ básico não encontrado:', line.substring(0, 100));
            continue;
          }
          
          // Inserir empresa primeiro (se não existir)
        ...
        console.log(`Lote processado com sucesso. Total processado: ${processedCount}`);
